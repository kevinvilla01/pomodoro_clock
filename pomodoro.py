...
class PomodoroClock:
    ...
    def __init__(self):
        ...
        self.root.tk.call('wm', 'iconphoto', self.root._w, PhotoImage(file="png-transparent-chibi-hatsune-miku.png"))
        ...
        self.sbreak_timer_label = ttk.Label(self.tab2, text="05:00", font=("Ubuntu", 48))
        self.sbreak_timer_label.pack(pady=20)

        self.lbreak_timer_label = ttk.Label(self.tab3, text="15:00", font=("Ubuntu", 48))
        self.lbreak_timer_label.pack(pady=20)
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            while full_s > 0 and not self.stopped:
                minutes, seconds = divmod(full_s, 60)
            ...
            while full_s > 0 and not self.stopped:
                minutes, seconds = divmod(full_s, 60)
                self.sbreak_timer_label.config(text=f"{minutes:02d}:{seconds:02d}")
                self.root.update()
                time.sleep(1)
                full_s -= 1
            ...
            while full_s > 0 and not self.stopped:
                minutes, seconds = divmod(full_s, 60)
                self.lbreak_timer_label.config(text=f"{minutes:02d}:{seconds:02d}")
                self.root.update()
                time.sleep(1)
                full_s -= 1
    ...
    def reset_timer(self):
        ...
        self.sbreak_timer_label.config(text="05:00")
        self.lbreak_timer_label.config(text="15:00")
